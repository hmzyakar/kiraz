...
public:
    KwFunc(const Node::Ptr func_name, const Node::Ptr func_arg_list, const Node::Ptr type_annot,
            const Node::Ptr func_scope)
    ...
    std::string as_string() const override {
        ...
        return fmt::format("Func(n={}, a={}, r={}, s={})", func_name_as_string,
                func_arg_list_as_string, type_annot_as_string, m_func_scope_as_string);
    }
...
public:
    ...
    std::string as_string() const override {
        ...
        for (size_t i = 0; i < m_identifiers.size(); i++) {
            func_arg_string += "FArg(n=" + m_identifiers[i]->as_string()
                    + ", t=" + m_types_list[i]->as_string() + ")";
...
public:
    ...
    std::string as_string() const override {
        ...
        if (m_repeat == nullptr) {
            ...
            repeat_as_string = m_repeat->as_string();
        }
...
public:
    KwIf(Node::Ptr if_check, Node::Ptr else_if_check, Node::Ptr if_scope, Node::Ptr else_if_scope,
            Node::Ptr else_scope)
            : m_if_check(if_check)
            , m_else_if_check(else_if_check)
            , m_if_scope(if_scope)
    ...
    std::string as_string() const override {
        ...
        if (m_if_scope == nullptr) {
            if_scope_as_string = "[]";
        }
        else {
            if_scope_as_string = m_if_scope->as_string();
        }
        if (m_else_scope == nullptr) {
            else_scope_as_string = "[]";
        }
        else {
            else_scope_as_string += m_else_scope->as_string();
        }
        return fmt::format("If(?={}, then={}, else={})", if_check_as_string, if_scope_as_string,
                else_scope_as_string);
    }
...
public:
    KwCall(const Node::Ptr func_name, const Node::Ptr arg_list)
            : m_func_name(func_name), m_arg_list(arg_list) {}
    std::string as_string() const override {
        std::string func_name_as_string = m_func_name->as_string();
...
public:
    CallArgList(Node::Ptr identifier) : Node() { m_identifiers.push_back(identifier); }
